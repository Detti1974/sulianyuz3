...
export default function BiologiaTopicMenu() {
  const { topicId } = useLocalSearchParams<{ topicId?: string }>();
  const tId = String(topicId || "");

  // Cím feloldása a bankból (ha megvan)
  const title =
    (BIO_TREE as any[]).find((t) => t.id === tId)?.title || "Biológia — téma";
  ...
  // ── Navigáció ────────────────────────────────────────────────
// ── Navigáció (DROP-IN, működik és nem dob TS hibát) ─────────────────
const goLearn = () =>
  (router as any).push(`/biologia/learn?topicId=${encodeURIComponent(tId)}`);

const goSubquiz = () =>
  (router as any).push(`/biologia/quiz-subtopic?topicId=${encodeURIComponent(tId)}`);

const startExamNow = () => {
  startExam(tId);
  (router as any).push("/biologia/quiz");
};
    ...
    <View style={styles.screen}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          ...
          {/* Dolgozat badge-ek – többsoros tördeléssel */}
          <View
            style={{
              flexDirection: "row",
              flexWrap: "wrap",
