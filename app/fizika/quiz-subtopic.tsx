...
import lessonToBank from "../../lib/fizika/lesson-map";
import styles from "../../lib/styles";

type Section = { id: string; title?: string };

const CUSTOM_SUB_ID = "__custom__";
const keyUserBank = (topicId: string) => `userbank:${topicId}`; // ⇐ egyezik a QuizContext-tel!

/* ===== util: fizika topic azonosító feloldás ===== */
const norm = (s: string) =>
  String(s || "")
    .toLowerCase()
    .replace(/^fizika_?\d*_?/, "")
...
function resolvePhysTopic(anyId: string) {
  const id = String(anyId || "");
  let t = (PHYS_TREE as any[]).find((x) => x.id === id);
  if (t) return t;
  const n = norm(id);
  t = (PHYS_TREE as any[]).find((x) => norm(x.id) === n);
  if (t) return t;
  t = (PHYS_TREE as any[]).find((x) => norm(x.id).endsWith(n));
  return t || null;
}

export default function FizikaQuizSubtopic() {
  const { topicId } = useLocalSearchParams<{ topicId?: string }>();
  const tId = String(topicId || "");
  const { startSub, getSubCount, resetSub } = useQuiz();

  const topic = React.useMemo(() => resolvePhysTopic(tId), [tId]);
  ...
  const openCustomEditor = () =>
    router.push({ pathname: "../fizika/custom-bank", params: { topicId: topic?.id ?? tId } });
        ...
        >
          ...
          <View style={{ marginTop: 10 }}>
            <TouchableOpacity
              ...
              style={[
                styles.btn,
                { backgroundColor: "#22c55e", borderColor: "#4ade80", opacity: busy ? 0.6 : 1 } // zöld
              ]}
            >
            ...
            <TouchableOpacity
              ...
              style={[
                styles.btn,
                { marginTop: 10, backgroundColor: "#8b5cf6", borderColor: "#a78bfa" } // lila
              ]}
            >
          ...
          >
            ...
            <View style={{ flexDirection: "row", gap: 10, marginTop: 8, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <TouchableOpacity
                ...
                style={[styles.btn, { backgroundColor: "#f97316", borderColor: "#fb923c" }]} // narancs
              >
            ...
            <TouchableOpacity
              ...
              style={[
                styles.btn,
                { marginTop: 10, backgroundColor: "#06b6d4", borderColor: "#22d3ee", opacity: busy ? 0.6 : 1 } // türkiz
              ]}
            >
              <Text style={[styles.buttonText, { color: "#fff" }]}>🧪 Indítás (10 kérdés)</Text>
            </TouchableOpacity>
