...
import { TOPIC_TREE as BANK } from "../../lib/fizika/bank";
...
function getLessonsTopic(topicId?: string | null): LessonsTopic | null {
  ...
  return (L[topicId] || L[`fizika_8_${topicId}`] || L[`fizika_${topicId}`]) ?? null;
}

function getBankTopic(topicId?: string | null) {
  if (!topicId) return null;
  const T: any = BANK as any;
  return T[topicId] || T[`fizika_8_${topicId}`] || T[`fizika_${topicId}`] || null;
}
...
export default function LearnFizika() {
  ...
  if (!subId) {
    ...
    const bankSections: { id: string; title: string }[] = (
      bankTopic?.sections || []
    ).map((s: any) => ({
    ...
    const headerTitle =
      topic?.title || bankTopic?.title || "Fizika — Tananyag";
  ...
  const mainTitle =
    lessonTopic?.title || bankTopic?.title || "Fizika — Tananyag";
          ...
          {content.length ? (
            ...
            <View style={[styles.block, styles.emptyBox]}>
              <Text style={styles.paragraph}>
                ...
                kvíz/dolgozat működik, mert a bankból jön — ide csak
                ugyanazzal az{" "}
                <Text style={{ fontWeight: "800" }}>altéma-ID-val</Text> adj
...
const styles = StyleSheet.create({
  ...
  noteText: { color: "#e5e7eb", fontSize: 15 },
  noteActions: { flexDirection: "row", gap: 8, marginTop: 4 },
