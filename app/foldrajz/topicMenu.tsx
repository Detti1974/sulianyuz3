...
export default function FoldrajzTopicMenu() {
  const { topicId } = useLocalSearchParams<{ topicId?: string }>();
  const tId = String(topicId || "");

  // Cím feloldása a bankból (ha megvan)
  const title =
    (GEO_TREE as any[]).find((t) => t.id === tId)?.title || "Földrajz — téma";

  const { startExam, getTopicCount } = useQuiz();

  // ✅ Csak a DOLGOZAT átmenők számlálója (≥50%)
  const [passes, setPasses] = React.useState(0);
  React.useEffect(() => {
    let alive = true;
    (async () => {
      try {
        const n = await getTopicCount(tId); // dolgozat-számláló
  ...
  // ── Navigáció ────────────────────────────────────────────────
  const goLearn = () =>
    (router as any).push(`/foldrajz/learn?topicId=${encodeURIComponent(tId)}`);

  const goSubquiz = () =>
    (router as any).push(`/foldrajz/quiz-subtopic?topicId=${encodeURIComponent(tId)}`);

  const startExamNow = () => {
    startExam(tId);
    (router as any).push("/foldrajz/quiz");
  };
    ...
    <View style={styles.screen}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          <Text style={[styles.titleBig, { color: "#06b6d4" }]}>{title}</Text>
        ...
        <View style={styles.card}>
          <TouchableOpacity
            ...
            style={[styles.btn, { backgroundColor: "#f08d23ff", borderColor: "#e6ab6bff" }]}
          >
          ...
          <TouchableOpacity
            ...
            style={[
              styles.btn,
              { marginTop: 10, backgroundColor: "#3b82f6", borderColor: "#60a5fa" },
            ]}
          >
