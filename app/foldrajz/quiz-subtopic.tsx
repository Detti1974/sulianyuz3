// app/foldrajz/quiz-subtopic.tsx
...
// A biológiában default export volt. Itt is rugalmasan kezeljük:
import { LESSON_MAP as lessonMapAny } from "../../lib/foldrajz/lesson-map";
...
/* ===== util: földrajz topic azonosító feloldás ===== */
/* Rugalmas: foldrajz_8_..., foldrajz8_..., geo_8_..., geo8_... stb. */
const norm = (s: string) =>
  String(s || "")
    .toLowerCase()
    .replace(/^foldrajz_?\d*_?/, "")
    .replace(/^geo_?\d*_?/, "")
    .replace(/_/g, "");

function resolveGeoTopic(anyId: string) {
  const id = String(anyId || "");
  let t = (GEO_TREE as any[]).find((x) => x.id === id);
  if (t) return t;

  const n = norm(id);
  t = (GEO_TREE as any[]).find((x) => norm(x.id) === n);
  if (t) return t;

  t = (GEO_TREE as any[]).find((x) => norm(x.id).endsWith(n));
  return t || null;
}

export default function FoldrajzQuizSubtopic() {
  const { topicId } = useLocalSearchParams<{ topicId?: string }>();
  const tId = String(topicId || "");
  const { startSub, getSubCount, resetSub } = useQuiz();

  const topic = React.useMemo(() => resolveGeoTopic(tId), [tId]);

  // lesson-map kompatibilitás (default vagy named export)
  const lessonMap = (lessonMapAny as any)?.default ?? (lessonMapAny as any) ?? {};
  const lmEntry = (lessonMap as any)?.[tId];
  ...
  const sections: Section[] = React.useMemo(() => {
    ...
    if (lmEntry?.sections?.length) {
      return (lmEntry.sections as string[]).map((sid: string) => ({ id: sid, title: sid }));
    }
    return [];
  }, [topic, lmEntry]);
