...
import lessonToBank from "../../lib/kemia/lesson-map";
...
/* ===== util: kémia topic azonosító feloldás ===== */
const norm = (s: string) =>
  String(s || "")
    .toLowerCase()
    .replace(/^kemia_?\d*_?/, "")
...
function resolveChemTopic(anyId: string) {
  const id = String(anyId || "");
  let t = (CHEM_TREE as any[]).find((x) => x.id === id);
  if (t) return t;
  const n = norm(id);
  t = (CHEM_TREE as any[]).find((x) => norm(x.id) === n);
  if (t) return t;
  t = (CHEM_TREE as any[]).find((x) => norm(x.id).endsWith(n));
  return t || null;
}

export default function KemiaQuizSubtopic() {
  const { topicId } = useLocalSearchParams<{ topicId?: string }>();
  const tId = String(topicId || "");
  const { startSub, getSubCount, resetSub } = useQuiz();

  const topic = React.useMemo(() => resolveChemTopic(tId), [tId]);
  ...
  const openCustomEditor = () =>
    router.push({ pathname: "/kemia/custom-bank", params: { topicId: topic?.id ?? tId } });
