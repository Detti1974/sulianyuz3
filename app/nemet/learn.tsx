...
import LESSONS from "../../lib/nemet/lessons";
import { TOPIC_TREE as BANK } from "../../lib/nemet/bank";
...
function findLessonContent(topicId: string | undefined, subId: string, subTitle?: string) {
  ...
  const topic = L?.[topicId!] || L?.[`nemet_8_${topicId}`] || L?.[`nemet_${topicId}`];
...
const PASTEL = ["#93c5fd", "#86efac", "#fde68a", "#f9a8d4", "#fca5a5", "#c7d2fe", "#7dd3fc"];
const RAINBOW = PASTEL;
...
function renderStyledText(rawText: string) {
  ...
  return parts.map((part, i) => {
    ...
    if (m) {
      const inner = m[1];
      const color = PASTEL[boldIdx % PASTEL.length];
      boldIdx++;
      return (
  ...
  return (
    <>
      {content.map((blk: any, i: number) => {
        const paragraphColor = PASTEL[i % PASTEL.length];
...
/* ===== ALTÉMA JEGYZET – ✏️/🗑 + billentyűzetbarát ===== */
...
function NotePad({ topicId, subId }: { topicId?: string | null; subId?: string | null }) {
  ...
  const key = `notes:nemet:${String(topicId ?? "")}:${String(subId ?? "")}`;
  ...
  const saveEdit = async () => {
    ...
    if (!t) {
      ...
      await persist(items.map((x) => (x.id === editId ? { ...x, text: t, ts: Date.now() } : x)));
    }
    setEditId(null);
    setText("");
    Keyboard.dismiss();
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ marginTop: 12 }}>
      <View
        style={{
          padding: 12,
          borderRadius: 14,
      ...
      >
        <Text style={{ color: "#7dd3fc", fontWeight: "900", fontSize: 20, marginBottom: 6 }}>
          Jegyzetek
        </Text>
...
/* ===== Képernyő ===== */
export default function LearnNemet() {
  ...
  if (!bankTopic) {
    return (
      <View style={styles.container}>
        <Text style={styles.screenTitle}>Német — Tananyag</Text>
        <View style={[styles.block, styles.emptyBox]}>
          <Text style={[styles.paragraph, { color: "#e2e8f0" }]}>Nem találom a főtémát a bankban.</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={80}>
      ...
      >
        ...
        {content.length ? <ContentBlocks content={content} /> : (
          <View style={[styles.block, styles.emptyBox]}>
            <Text style={[styles.paragraph, { color: "#e2e8f0" }]}>Ehhez az altémához még nincs tananyag a lessons.ts-ben.</Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  screenTitle: { fontSize: 28, fontWeight: "900", color: "#38bdf8" },
  backLink: { color: "#94a3b8", fontSize: 16 },

  card: { borderRadius: 16, padding: 16 },
  cardTitle: { color: "#e5e7eb", fontSize: 22, fontWeight: "800" },

  block: { borderRadius: 16, padding: 16, borderWidth: 1, borderColor: "rgba(147,197,253,0.25)" },
