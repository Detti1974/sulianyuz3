...
import { TOPIC_TREE as NEMET_TREE } from "../../lib/nemet/bank";
import lessonToBank from "../../lib/nemet/lesson-map";
...
/* ===== util: német topic azonosító feloldás ===== */
const norm = (s: string) =>
  String(s || "")
    .toLowerCase()
    // rugalmas: nemet_8_..., nemet8_..., de_8_..., de8_...
    .replace(/^nemet_?\d*_?/, "")
    .replace(/^de_?\d*_?/, "")
    .replace(/_/g, "");

function resolveNemetTopic(anyId: string) {
  const id = String(anyId || "");
  let t = (NEMET_TREE as any[]).find((x) => x.id === id);
  if (t) return t;

  const n = norm(id);
  t = (NEMET_TREE as any[]).find((x) => norm(x.id) === n);
  if (t) return t;

  t = (NEMET_TREE as any[]).find((x) => norm(x.id).endsWith(n));
  return t || null;
}

export default function NemetQuizSubtopic() {
  const { topicId } = useLocalSearchParams<{ topicId?: string }>();
  const tId = String(topicId || "");
  const { startSub, getSubCount, resetSub } = useQuiz();

  const topic = React.useMemo(() => resolveNemetTopic(tId), [tId]);
    ...
    <View style={styles.screen}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={[styles.card, shadow]}>
          <Text style={[styles.titleBig, { color: "#f59e0b" }]}>{topicTitle}</Text>
          <Text style={[styles.paragraph, { marginTop: 6 }]}>
            Válaszd ki az{" "}
            <Text style={{ fontWeight: "800", color: "#fbbf24" }}>alfejezetet</Text>, és indul a 10 kérdéses kvíz.
          </Text>
        </View>
        ...
        <View
          style={[
            styles.card,
            shadow,
            { backgroundColor: "rgba(251,191,36,0.08)", borderWidth: 1, borderColor: "rgba(251,191,36,0.4)" },
          ]}
        >
          <Text style={[styles.title, { color: "#facc15" }]}>Saját kvíz</Text>
          <Text style={[styles.paragraph, { marginTop: 6 }]}>
        ...
        {sections.map((s) => (
          ...
          >
            <Text style={[styles.title, { color: "#fbbf24" }]}>{s.title}</Text>
            ...
            <TouchableOpacity
              ...
              style={[
                styles.btn,
                { marginTop: 10, backgroundColor: "#f59e0b", borderColor: "#fbbf24", opacity: busy ? 0.6 : 1 },
              ]}
            >
              <Text style={[styles.buttonText, { color: "#fff" }]}>🧠 Indítás (10 kérdés)</Text>
            </TouchableOpacity>
